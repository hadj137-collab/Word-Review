...
st.caption("自動讀取 Google Drive 中的 Test 單字表 ＋ Score 篩選")

# === 核心邏輯：自動從雲端下載 Excel/CSV 資料 ===
@st.cache_data(ttl=600)  # 快取機制：10分鐘內不重複下載，加速載入；10分鐘後會自動重新抓取最新資料
def load_data_from_drive(url):
    try:
        ...
        st.error(f"連線至 Google Drive 失敗，請檢查網址是否正確或檔案是否已發布。錯誤訊息: {e}")
        return None
...
if df is not None:
    ...
    if mode == "🃏 單字卡模式":
        ...
        with st.container(border=True):
            st.markdown(f"<h1 style='text-align: center; color: #4A90E2;'>{current_vocab['Word']}</h1>", unsafe_allow_html=True)
            ...
            if st.session_state.show_definition:
                st.write(f"**💡 Sentence：**")
                st.info(current_vocab['Sentence'])
            else:
                st.write("*(點擊下方按鈕查看完整句子)*")
